...
class LeggedRobot(BaseTask):
    ...
    def compute_self_and_task_obs(self, ):
        ...
        if self.cfg.motion.teleop:
            if self.cfg.motion.teleop_obs_version == 'v1':
            ...
            elif self.cfg.motion.teleop_obs_version == 'v-teleop-extend-max-nolinvel-history':
                body_pos = self._rigid_body_pos
                body_rot = self._rigid_body_rot
                body_vel = self._rigid_body_vel
                body_ang_vel = self._rigid_body_ang_vel
                ...
                obs = torch.cat([dof_pos, dof_vel, base_acc, base_ang_vel, base_gravity,  # 19dim + 19dim + 3dim + 3dim + 3dim 
                                            task_obs,  # 
                                            self.actions], dim = -1) # 19dim                
            elif self.cfg.motion.teleop_obs_version == 'v-teleop-dof-nolinvel-history':
                with torch.no_grad():    
                    # robot
                    dof_pos = self.dof_pos
                    dof_vel = self.dof_vel
        ...
        if self.add_noise:
            ...
            if self.cfg.motion.teleop_obs_version == 'v-teleop-extend-max-nolinvel-history':
                # TODO
                # 研究怎么只给最后一次观测加噪声
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        if self.cfg.motion.teleop:
            ...
            if self.cfg.motion.teleop_obs_version == 'v1':
                ...
                # local_body_pos.shape, local_body_rot_obs.shape, local_body_vel.shape, local_body_ang_vel.shape, dof_pos.shape, dof_vel.shape
                # local_body_pos 3x19
                noise_vec[-135                   : -135 + self.num_dof      ] = noise_scales.dof_pos * noise_level * self.obs_scales.dof_pos
                # dof vel
                noise_vec[-135 + self.num_dof    : -135 + 2*self.num_dof    ] = noise_scales.dof_vel * noise_level * self.obs_scales.dof_vel
                # base ang vel
                noise_vec[-135 + 2*self.num_dof + 3 - 3 : -135 + 2*self.num_dof + 6 - 3] = noise_scales.ang_vel * noise_level * self.obs_scales.ang_vel
                # base gravity
                noise_vec[-135 + 2*self.num_dof + 6 - 3 : -135 + 2*self.num_dof + 9 - 3] = noise_scales.gravity * noise_level
                # ref dof pos
                noise_vec[-135 + 2*self.num_dof + 9 - 3 : -135 + 2*self.num_dof + 9 + (len(self.cfg.motion.teleop_selected_keypoints_names) + 2) *3 * 3 - 3 ] = noise_scales.ref_body_pos * noise_level * self.obs_scales.body_pos                    
            elif self.cfg.motion.teleop_obs_version == 'v-teleop-extend-max-acc':
                # local_body_pos.shape, local_body_rot_obs.shape, local_body_vel.shape, local_body_ang_vel.shape, dof_pos.shape, dof_vel.shape
                # local_body_pos 3x19
                noise_vec[0                   : self.num_dof      ] = noise_scales.dof_pos * noise_level * self.obs_scales.dof_pos
                ...
                noise_vec[-113 + 0                  : -113 + self.num_dof] = noise_scales.dof_pos * noise_level * self.obs_scales.dof_pos
                # dof vel
                noise_vec[-113 + self.num_dof        : -113 + 2*self.num_dof    ] = noise_scales.dof_vel * noise_level * self.obs_scales.dof_vel
                # base vel
                # noise_vec[-113 + 2*self.num_dof      : -113 + 2*self.num_dof + 3] = noise_scales.lin_vel * noise_level * self.obs_scales.lin_vel
                # base ang vel
                noise_vec[-113 + 2*self.num_dof + 3 -3 : -113 + 2*self.num_dof + 6-3] = noise_scales.ang_vel * noise_level * self.obs_scales.ang_vel
                # base gravity
                noise_vec[-113 + 2*self.num_dof + 6 -3 : -113 + 2*self.num_dof + 9-3] = noise_scales.gravity * noise_level
                # x y heading targets
                noise_vec[-113 + 2*self.num_dof + 9 -3 : -113 + 2*self.num_dof + 12-3] = 0.
                
                # ref dof pos
                noise_vec[-113 + 2*self.num_dof + 12 -3: -113 + 3*self.num_dof + 12-3] = noise_scales.ref_dof_pos * noise_level * self.obs_scales.dof_pos
                # ref dof vel
                noise_vec[-113 + 3*self.num_dof + 12 -3: -113 + 4*self.num_dof + 12-3] = noise_scales.ref_dof_vel * noise_level * self.obs_scales.dof_vel
                # ref base vel
                noise_vec[-113 + 4*self.num_dof + 12 -3: -113 + 4*self.num_dof + 15-3] = noise_scales.ref_lin_vel * noise_level * self.obs_scales.lin_vel
                # ref base ang vel
                noise_vec[-113 + 4*self.num_dof + 15 -3: -113 + 4*self.num_dof + 18-3] = noise_scales.ref_ang_vel * noise_level * self.obs_scales.ang_vel
                # ref base gravity
                noise_vec[-113 + 4*self.num_dof + 18 -3: -113 + 4*self.num_dof + 21-3] = noise_scales.ref_gravity * noise_level
